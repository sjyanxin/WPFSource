...
namespace MS.Internal.Data 
{
    internal sealed class PropertyPathWorker: IWeakEventListener 
    {
        ...
        internal ValidationError ValidateDataError(BindingExpressionBase bindingExpressionBase)
        {
            ValidationError result = null; 
            int level = Length - 1;
 
            if (level >= 0 && SVI[level].type == SourceValueType.Property) 
            {
        ...
        // Coded to avoid loading System.Xml until necessary. 
        bool IsEmptyXmlDataCollection(object parent)
        { 
            return AssemblyHelper.IsLoaded(UncommonAssembly.System_Xml) 
                    ? IsEmptyXmlDataCollectionImpl(parent)
                    : false; 
        }

        // separate method to avoid loading System.Xml
        [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.NoInlining)] 
        bool IsEmptyXmlDataCollectionImpl(object parent)
        { 
        ...
        { 
            ...
            case SourceValueType.Indexer:
                ...
                if (info == null && AssemblyHelper.IsIDynamicMetaObjectProvider(item)) 
                {
                    if (MatchIndexerParameters(null, aryInfo, args, false)) 
                    { 
                        info = DynamicIndexerAccessor.GetIndexerAccessor(args.Length);
        ...
        private bool ShouldConvertIndexerToProperty(object item, ref string name) 
        {
            ...
            if (AssemblyHelper.IsDataRowView(item)) 
            {
