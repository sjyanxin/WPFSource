...
namespace System.Windows.Baml2006 
{
    ...
    { 
        ...
        {
            get { return _context.CurrentFrame.LineNumber; }
        } 

        int IXamlLineInfo.LinePosition 
        { 
            get { return _context.CurrentFrame.LineOffset; }
        } 
        ...
        {
            _context.CurrentFrame.LineOffset = _binaryReader.ReadInt32();
            ...
            if (consumer != null) 
            { 
                consumer.SetLineInfo(_context.CurrentFrame.LineNumber, _context.CurrentFrame.LineOffset);
            } 
        }
        ...
        {
            _context.CurrentFrame.LineNumber = _binaryReader.ReadInt32(); 
            _context.CurrentFrame.LineOffset = _binaryReader.ReadInt32(); 
            // We do this cast on every line info, but that is harmless for perf since line info is only in debug build
            IXamlLineInfoConsumer consumer = _xamlNodesWriter as IXamlLineInfoConsumer; 
            if (consumer != null)
        ...
        private string GetStaticExtensionValue(short valueId, out Type memberType, out object providedValue) 
        {
            ...
            if (valueId < 0) 
            {
                ...
                // else it is the key.
                if (valueId > (short)System.Windows.SystemResourceKeyID.InternalSystemThemeStylesEnd)
                { 
                    valueId -= (short)System.Windows.SystemResourceKeyID.InternalSystemThemeStylesEnd;
                    isKey = false; 
                } 

                if (valueId > (short)SystemResourceKeyID.InternalSystemColorsStart && valueId < (short)SystemResourceKeyID.InternalSystemThemeStylesEnd) 
                {
                    if (_isBinaryProvider)
                    {
                        if (isKey) 
